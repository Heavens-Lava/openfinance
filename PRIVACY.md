...
OpenFinance processes selected bank CSV files locally. The public deployment does not connect to financial institutions or transmit imported financial contents to an OpenFinance application server. Imported files are persisted in the current browser or native app's IndexedDB until cleared, while preferences and user-defined rules may use localStorage.
